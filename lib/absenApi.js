...
export function absenStatus(res) {
    ...
    // Sumber utama expired time adalah top-level response dari PHP (wa_absen_status_meta),
    // fallback ke record hanya untuk kompatibilitas jika PHP lama belum mengirim expires_at.
    const exp = Number(res?.expires_at ?? record?.expires_at ?? 0)
    const remainingSec = Number(res?.remaining_seconds ?? Math.max(0, exp - now))
    const active = !!record && (res?.expired !== undefined ? !res.expired : exp > now)
    return { record, now, exp, active, remainingSec }
}
