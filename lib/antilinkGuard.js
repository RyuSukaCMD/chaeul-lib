...
export async function enforceAntilink(sock, m, { isCreator }) {
    ...
    // Premium FULL kebal (Basic TIDAK kebal antilink)
    const senderPn = await resolvePn(sock, m, m.sender)
    if (isFullPremium(senderPn) || isFullPremium(m.sender)) return false
