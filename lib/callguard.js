...
//
//  SEMUA panggilan masuk dicatat ke database/calllog.json (nomor
//  penelepon, waktu, video/voice, aksi yang diambil).
//
//  Perilaku saat blockcall aktif:
    ...
    sock.ev.on("call", async (calls) => {
        try {
            for (const call of calls || []) {
                ...
                const on = !!global.settings?.blockcall
                const action = on && !isOwner ? "rejected+blocked" : "observed"

                // Selalu dicatat — fitur ON maupun OFF.
                try {
                    logCall(from, { action, isVideo: !!call.isVideo })
                } catch {}

                if (!on || isOwner) continue
