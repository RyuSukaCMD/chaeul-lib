...
async function broadcastWeather(sock, weatherList) {
    ...
    for (const jid of getWeatherGroups()) {
        // Notifikasi weather dimatikan untuk grup ini (.disable weather) → skip.
        if (!canNotify(jid, "weather")) continue
        try {
...
async function startScheduledWeather(sock) {
    // Sistem dinonaktifkan lewat .disable → jangan spawn/broadcast weather baru.
    // (Jadwal watcher tetap berputar; begitu di-enable lagi langsung jalan.)
    if (!isSystemEnabled("weather")) return
