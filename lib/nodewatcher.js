...
export async function runNodeCheckOnce(sock, { notifyTargets = true } = {}) {
    ...
    // Hormati setelan notifikasi per-grup (.disable nodewarn).
    const targets = getWarnTargets().filter((jid) => canNotify(jid, "nodewarn"))
...
export function startNodeWatcher(sock) {
    ...
    const tick = async () => {
        // Sistem dinonaktifkan lewat .disable → lewati cek (jadwal tetap berjalan,
        // jadi begitu di-enable lagi langsung aktif tanpa restart).
        if (isSystemEnabled("nodewarn") && !running) {
            running = true
            try {
