    ...
    const extraInfo =
        action === "whitelist"
            ? [
                  ...
                  ``,
                  `_🔴 Node offline/maintenance tidak bisa di-whitelist_`,
                  `_(bisa dipaksa lewat ⚡ Force Use saat ditolak)._`,
            ...
            : [
                  ...
                  `_Mode aktif: ${mode === "whitelist" ? "⚪ WHITELIST" : "⚫ BLACKLIST"}_`,
                  mode === "blacklist"
                      ? `_Mode blacklist: node ter-blacklist tidak bisa di-unblacklist_`
                      : ``,
                  mode === "blacklist" ? `_(bisa dipaksa lewat ⚡ Force Use saat ditolak)._` : ``
              ]
...
// force=true (tombol ⚡ Force Use) memaksa-mengizinkan node untuk urgent:
//  • whitelist → whitelist tetap ditambah walau node offline/maintenance
//  • blacklist → unblacklist dipaksa walau mode blacklist aktif
export async function applyNodePick({ sock, m, action, nodeId, force = false }) {
    const prefix = PREFIX[action]
    ...
    const replyCard = (title, lines, emoji, extraButtons = []) =>
        Button.menu({
            sock,
            m,
    ...
    if (action === "whitelist") {
        ...
        // Force pada node yang sudah whitelisted = no-op (jangan ke-toggle hapus).
        if (isNodeWhitelisted(id)) {
            if (force) {
                return replyCard(
                    "SUDAH WHITELIST",
                    [`ℹ️ Node *${name}* (ID: ${id}) memang sudah ada di whitelist.`],
                    "ℹ️"
                )
            }
        ...
        // (kecuali dipaksa lewat tombol ⚡ Force Use)
        if (!liveInfo.online) {
            if (!force) {
                return replyCard(
                    ...
                    [
                        ...
                        ``,
                        `_Atau paksa dengan tombol ⚡ Force Use di bawah._`
                    ],
                    "🚫",
                    [{ type: "quick", text: "⚡ Force Use", id: `${prefix}_force:${id}` }]
                )
            }

            // ─── ⚡ FORCE USE: whitelist dipaksa walau offline/maintenance ───
            whitelistNode(id)
            const forceLines = [
                `⚡ Node *${name}* (ID: ${id}) *DIPAKSA* whitelist.`,
                ``,
                `Status node saat ini: ${statusLine}`,
                ``,
                `⚠️ WARNING: node masih ${liveInfo.maintenance ? "maintenance" : "offline"} —`,
                `claim dari node ini berisiko gagal/error.`,
                ``
            ]
            if (mode !== "whitelist") {
                forceLines.push(`ℹ️ Mode saat ini *BLACKLIST* — whitelist belum berpengaruh.`)
                forceLines.push(`_Ubah dengan ${global.prefix}urgentmode whitelist_`)
            } else {
                forceLines.push(`✅ Node ini sekarang *DIIZINKAN* untuk urgent (mode whitelist).`)
            }
            return replyCard("⚡ FORCE WHITELIST", forceLines, "⚡")
        }
    ...
    if (isNodeBlacklisted(id)) {
        ...
        if (mode === "blacklist") {
            if (!force) {
                return replyCard(
                    ...
                    [
                        ...
                        `_Atau paksa izinkan dengan tombol ⚡ Force Use._`
                    ],
                    "🚫",
                    [{ type: "quick", text: "⚡ Force Use", id: `${prefix}_force:${id}` }]
                )
            ...
            // ─── ⚡ FORCE USE: unblacklist dipaksa dalam mode blacklist ───
            removeBlacklistNode(id)
            return replyCard(
                "⚡ FORCE ALLOW",
                [
                    `⚡ Node *${name}* (ID: ${id}) *DIPAKSA* diizinkan untuk urgent.`,
                    ``,
                    `Blacklist node ini dicabut walau *mode blacklist* aktif.`,
                    `Node sekarang *boleh* di-claim dengan ${global.prefix}urgent.`,
                    ``,
                    `⚠️ WARNING: ini override manual dari aturan mode blacklist.`,
                    `Mode tetap *BLACKLIST* — node bisa di-blacklist lagi kapan saja.`
                ],
                "⚡"
            )
        }
