...
import { isSystemEnabled } from "./systems.js"
import { canNotify } from "./groupsilence.js"
...
export async function runAbsentWarnOnce(sock) {
    if (!sock) return
    // Sistem dinonaktifkan lewat .disable → tidak mengirim peringatan apa pun.
    if (!isSystemEnabled("absentwarn")) return
    const data = db()
    ...
    for (const chat of groups) {
        // Notifikasi absen dimatikan untuk grup ini (.disable absentwarn) → skip.
        if (!canNotify(chat, "absentwarn")) continue
        ...
        for (const p of meta?.participants || []) {
            ...
            // Premium tidak perlu absen dan tidak perlu di-warn.
            if (res?.premium || res?.code === "premium_skip" || res?.user?.role === "premium") continue
