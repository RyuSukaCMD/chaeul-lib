...
function fresh() {
    return {
        ...
        islands: { fisherman: true }, // island yang sudah terbuka
        enchant: null, // enchant aktif di rod (id) atau null
...
export function getPlayer(jid) {
    ...
    return {
        ...
        islands: { fisherman: true, ...(db[key].islands || {}) },
        pity: { ...(db[key].pity || {}) }
    }
}
...
export function hasIsland(jid, island) {
    if (island === "fisherman") return true
    return !!getPlayer(jid).islands?.[island]
}
export function unlockIsland(jid, island) {
    const p = getPlayer(jid)
    const islands = { ...p.islands, [island]: true }
    return updatePlayer(jid, { islands })
}

// ─── Enchant ───
export function getEnchantId(jid) {
    return getPlayer(jid).enchant || null
}
export function setEnchant(jid, enchantId) {
    return updatePlayer(jid, { enchant: enchantId })
}
