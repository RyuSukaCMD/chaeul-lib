...
import { isGroupSilent, isGroupBlacklisted, isGroupAllDisabled } from "./groupsilence.js"
import { applySilenceGuard, allowSilenceTemporarily } from "./silenceGuard.js"
...
// Command recovery KHUSUS OWNER yang tetap lolos walau grup diblacklist
// (satu-satunya cara menghidupkan bot lagi dari dalam grup).
const OWNER_RECOVER_CMDS = [
    "delblacklistgroup",
    "delbl",
    "blacklistgroup",
    "blgroup",
...
/** Ambil nama command dari body (menghormati prefix / noPrefix). */
function parseCommandName(body) {
    const raw = body.startsWith(global.prefix)
        ? body.slice(global.prefix.length)
        : global.noPrefix
          ? body
          : ""
    if (!raw) return ""
    return raw.trim().split(/ +/).shift().toLowerCase()
}
...
export default async function Handler(sock, m) {
    // Pasang penjaga output global (idempotent): semua pesan/presence/read
    // ke grup yang "mati total" otomatis dibuang, dari modul mana pun.
    try { applySilenceGuard(sock) } catch {}
    ...
    // ─── Grup Mati Total (Blacklist / Disable All) ───
    // Bot BENAR-BENAR diam total di grup ini: tidak ada command, tombol,
    // antilink, notif AFK, sesi, teks "grup belum terdaftar", dll.
    // Owner tetap bisa memakai command (recovery: .delbl / .enablecommand all).
    // Di grup "disable all" (bukan blacklist), command recovery tetap lolos
    // agar admin bisa menyalakan bot lagi.
    if (m.isGroup && isGroupSilent(m.chat)) {
        const cmdName = parseCommandName(body)

        if (isOwnerEarly) {
            // Owner: hanya command recovery yang boleh berjalan & bersuara.
            // Command lain diabaikan total agar grup benar-benar mati.
            if (!cmdName || !OWNER_RECOVER_CMDS.includes(cmdName)) return
            allowSilenceTemporarily(m.chat)
        } else if (isGroupBlacklisted(m.chat)) {
            // Grup blacklist → mati mutlak untuk semua non-owner.
            return
        } else if (!cmdName || !RECOVER_CMDS.includes(cmdName)) {
            // Grup "disable all" → hanya command recovery admin yang lolos.
            return
        } else {
            allowSilenceTemporarily(m.chat)
        }
    ...
    if (!m.isGroup && !m.fromMe && !isOwnerEarly) {
        ...
        if (global.settings?.blockpc) {
            if (!warnedBlockPc.has(senderNum)) {
                ...
                // Izinkan SATU peringatan lewat silence guard PC.
                allowSilenceTemporarily(m.chat, 10000)
                try {
                    await m.reply(
    ...
    // Catatan: gate blacklist & disable-all grup sudah ditangani TOTAL
    // di awal handler (grup mati total, lihat grup silence gate).
    ...
    // Trusted access di grup trust → bypass gate registrasi grup.
    if (m.isGroup && !isCreator && !isTrustedAccess) {
        ...
        // "Disable all" & kena silent gate total di awal handler; yang sampai
        // ke sini hanyalah command RECOVERY (atau owner) → biarkan jalan.
        if (isGroupAllDisabled(m.chat)) {
            // (isGroupAllDisabled → RECOVER_CMDS sudah diverifikasi di gate awal)
            ...
            // Command boleh jalan bila: masuk RECOVER_CMDS ATAU di-whitelist (isAllowed)
            if (
                isAllDisabled(m.chat) &&
                !RECOVER_CMDS.includes(command) &&
