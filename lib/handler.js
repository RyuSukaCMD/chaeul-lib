...
export default async function Handler(sock, m) {
    ...
    if (m.isGroup && !isOwnerEarly && isGroupSilent(m.chat)) {
        const recoverBody = body.startsWith(global.prefix)
            ? body.slice(global.prefix.length)
            : global.noPrefix
              ? body
              : ""
        const isRecoverCommand =
            recoverBody &&
            RECOVER_CMDS.includes(recoverBody.trim().split(/ +/).shift().toLowerCase())
        if (isGroupBlacklisted(m.chat) || !isRecoverCommand) return
    }
