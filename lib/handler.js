...
import { isTrustedGroup, isTrustedUser, isTrustBlockedCommand, touchTrustedGroupName } from "./trust.js"
...
import { logPrivateChat } from "./pclog.js"
...
export default async function Handler(sock, m) {
    ...
    if (!m.isGroup && !m.fromMe && !isOwnerEarly) {
        try {
            logPrivateChat(m.sender, m.pushName)
        } catch {}
        if (global.settings?.blockpc) {
        ...
        if (global.settings?.grouponly) return
    }
    ...
    // ─── Trusted Access ───
    // User trusted (.trust) di grup trusted (.trustgroup) = akses owner-level
    // untuk seluruh command (kecuali reboot & self), plus bypass register,
    // self-mode, dan gate grup-belum-terdaftar. Di luar grup trust → biasa.
    const isTrustedAccess =
        !isCreator && m.isGroup && isTrustedGroup(m.chat) && isTrustedUser(sender)
    ...
    if (global.settings?.public === false && !isCreator && !isAbsenEarly && !isTrustedAccess) return
    ...
    if (m.isGroup && !isCreator && !isTrustedAccess) {
        ...
        // (a) Blacklist / belum terdaftar → diam total untuk command.
        if (isGroupBlacklisted(m.chat)) {
            if (!RECOVER_CMDS.includes(primary) && !groupMgmtCmds.includes(primary)) return
        } else if (!isGroupRegistered(m.chat)) {
            if (!groupMgmtCmds.includes(primary) && !RECOVER_CMDS.includes(primary)) return
        }

        // (b) Disable command / disable all.
        if (isAllDisabled(m.chat)) {
            // Semua command mati; hanya command recovery & whitelist yang lolos.
            if (!RECOVER_CMDS.includes(primary) && !isAllowed(m.chat, primary)) return
        }

        // Cek per-command (alias & tombol ikut, karena pakai nama utama).
    ...
    if (!isCreator && !isRegistered(sender) && !bypassRegister && !isTrustedAccess) {
        if (global.settings?.autotyping || global.settings?.autovoice) {
            await sock.sendPresenceUpdate("paused", m.chat)
        }
        return Button.menu({
