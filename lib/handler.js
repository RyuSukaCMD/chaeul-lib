...
import { startNodeWatcher } from "./nodewatcher.js"
import { startCallGuard } from "./callguard.js"
import { isGroupBlacklisted } from "./groupsilence.js"
import { isTrustBlockedCommand, touchTrustedGroupName } from "./trust.js"
import { hasPortSession, handlePortInput, cancelPortSession } from "../command/owner/urgent.js"
import { getBulkSession, clearBulkSession } from "./bulkserverSession.js"
import { handleInput as handleBulkInput } from "../command/owner/bulkserver.js"
import { logPrivateChat, markPcBlocked } from "./pclog.js"
...
let nodeWatcherStarted = false
let callGuardStarted = false

// Command recovery yang TETAP lolos di grup kena "disable all"
// (agar grup bisa dinyalakan lagi oleh admin/owner).
const RECOVER_CMDS = [
    "registergroup",
    "reggroup",
    "unregistergroup",
...
// Nomor yang sudah diperingatkan soal block-pc (peringatan hanya sekali).
const warnedBlockPc = new Set()

export default async function Handler(sock, m) {
    // Pulihkan toggle tersimpan (grouponly / blockpc / blockcall / prefix)
    // sekali per proses, agar setting bertahan setelah restart walau bot
    // induk belum memanggil loadSettings().
    if (!settingsLoaded) {
        settingsLoaded = true
    ...
    if (!nodeWatcherStarted) {
        nodeWatcherStarted = true
        try { startNodeWatcher(sock) } catch {}
    }
    if (!callGuardStarted) {
        callGuardStarted = true
        try { startCallGuard(sock) } catch {}
    }
    ...
    // ─── Hak Akses ───
    // Dihitung SEKALI di awal memakai seluruh identitas pengirim
    // (sender / participant / participantAlt / @lid), supaya owner &
    // trusted tidak pernah salah dikenali sebagai user biasa.
    const access = resolveAccess(m)
    const isCreator = access.isCreator
    const isTrustedAccess = access.isTrusted
    const isPrivileged = access.isPrivileged
    const sender = access.sender || m.senderNumber || m.sender.split("@")[0]
    const senderNum = sender

    // ─── Catatan Gate Grup ───
    // Blacklist grup (.blgroup) & grup belum terdaftar TIDAK lagi membuat
    // bot mati total. Efeknya HANYA: command tidak dilayani (diam, tanpa
    // teks penolakan). Notifikasi, welcome, antilink, auto-read, auto-typing
    // dan sistem lain tetap berjalan normal.
    // Gate-nya dipasang di bagian command (lihat "Gate Command Grup").

    // ─── Aturan Private Chat (non-owner) ───
    // Semua PC dari non-owner DICATAT ke database/pclog.json (nomor,
    // nama, pertama/terakhir chat, jumlah pesan).
    // blockpc    : peringatkan SEKALI lalu block nomor.
    // grouponly  : bot hanya merespon di grup; PC diabaikan diam-diam.
    if (!m.isGroup && !m.fromMe && !isPrivileged) {
        const blockPc = !!global.settings?.blockpc
        const groupOnly = !!global.settings?.grouponly

        // Catat SETIAP private chat (nomor +62xxx, nama, jumlah pesan).
        try {
            logPrivateChat(m.sender, m.pushName, {
                action: blockPc ? "blocked" : groupOnly ? "ignored" : "logged"
            })
        } catch (e) {
            Logger.error?.(`pclog → ${e.message}`)
        }

        if (blockPc) {
            // Peringatan hanya SEKALI per nomor, lalu LANGSUNG diblokir.
            if (!warnedBlockPc.has(senderNum)) {
                warnedBlockPc.add(senderNum)
                try {
                    await m.reply(
                        `╭━━━〔 🚫 PRIVATE CHAT BLOCKED 〕━━━⬣\n` +
                            `┃\n` +
                            `┃ Bot tidak menerima chat private.\n` +
                            `┃ Gunakan bot di grup resmi.\n` +
                            `┃\n` +
                            `┃ 📱 ${toDisplay(m.sender)}\n` +
                            `┃ Nomor kamu telah diblokir.\n` +
                            `┃\n` +
                            `╰━━━━━━━━━━━━━━━━━━⬣`
                    )
                } catch {}
            ...
            // Block LANGSUNG (tidak menunggu pesan berikutnya).
            try {
                await sock.updateBlockStatus?.(m.sender, "block")
                markPcBlocked(m.sender)
                console.log(`[BlockPC] ${toDisplay(m.sender)} diblokir otomatis.`)
            } catch (e) {
                Logger.error?.(`blockpc → ${e.message}`)
            }
            return
        }
    ...
    if (m.isGroup && !m.fromMe && !isPrivileged && isUserMuted(m.chat, m.sender)) {
        try {
    ...
    // Catat nama grup trust (untuk tampilan .trustgroup).
    if (isTrustedAccess && m.groupName) {
        try { touchTrustedGroupName(m.chat, m.groupName) } catch {}
    }
    ...
    // ─── Bulk Server Session ───
    // Menangkap data akun multi-baris yang dikirim TANPA prefix setelah
    // owner/trusted mengetik .bulkserver.
    const bulkSession = getBulkSession(m.sender)
    if (bulkSession && bulkSession.step === "input" && body && !body.startsWith(global.prefix)) {
        const value = body.trim()
        if (["batal", "cancel", "stop"].includes(value.toLowerCase())) {
            clearBulkSession(m.sender)
            return m.reply("❌ Bulk create dibatalkan.")
        }
        try {
            return await handleBulkInput(sock, m, value)
        } catch (error) {
            clearBulkSession(m.sender)
            Logger.error?.(`bulkserver → ${error.message}`)
    ...
    // Trusted access di grup trust → bypass self-mode juga.
    if (global.settings?.public === false && !isPrivileged && !isAbsenEarly) return
    ...
    // ─── Gate Command Grup ───
    // Owner & trusted user selalu BYPASS seluruh gate di bawah ini.
    //
    //  a) Blacklist grup / grup belum terdaftar
    //     → bot hanya TIDAK MEMBALAS command (diam, tanpa teks penolakan).
    //       Notifikasi & sistem lain tidak terpengaruh.
    //     → Command pendaftaran/pemulihan grup tetap dilayani.
    //
    //  b) Disable command
    //     → bot TIDAK MELISTEN command tsb (diam). Mematikan sebuah command
    //       otomatis mematikan seluruh alias & sub-command (tombol)-nya,
    //       karena pengecekan memakai nama utama plugin.
    if (m.isGroup && !isPrivileged) {
        // Command pendaftaran grup — tetap boleh walau grup belum terdaftar.
        const groupMgmtCmds = ["registergroup", "reggroup", "unregistergroup", "unregroup"]

        // Nama utama plugin → dipakai agar alias & sub-command ikut mati.
        const primary = Loader.primaryName(plugin) || command
        const isRecover = RECOVER_CMDS.includes(primary) || groupMgmtCmds.includes(primary)

        // (a) Blacklist grup → diam untuk command (kecuali recovery).
        if (isGroupBlacklisted(m.chat)) {
            if (!isRecover) return
        }

        // (b) Grup belum terdaftar → diam untuk command (kecuali recovery).
        if (!isGroupRegistered(m.chat) && !isRecover) return

        // (c) Disable all → hanya recovery & command yang di-whitelist.
        if (isAllDisabled(m.chat) && !isRecover && !isAllowed(m.chat, primary)) return

        // (d) Command yang di-disable (alias & tombol ikut).
        if (isDisabled(m.chat, primary) || isDisabled(m.chat, command)) return

        // (e) Command khusus admin grup.
        if (isAdminOnly(m.chat, primary) || isAdminOnly(m.chat, command)) {
            const { isAdmin } = await checkAdmin(sock, m, isCreator)
            if (!isAdmin) {
                return m.reply(`🔒 Command *${primary}* khusus admin grup.`)
            }
        }
    }

    // ─── Anti-Spam Gate ───
    // Membolehkan burst wajar (6 command / 10 detik). Melebihi itu →
    // satu peringatan, lalu diam sampai cooldown singkat selesai.
    // Owner, trusted, dan klik tombol dikecualikan.
    if (!isPrivileged && !isButton) {
        ...
        if (spam.action === "warn") {
            ...
            return m.reply(
                `╭━━━〔 ⏳ TERLALU CEPAT 〕━━━⬣\n` +
                    `┃\n` +
                    `┃ Pelan-pelan ya! Tunggu ${spam.remaining} detik\n` +
                    `┃ sebelum memakai command lagi.\n` +
                    `┃\n` +
                    `╰━━━━━━━━━━━━━━━━━━⬣`
            )
        }

        if (spam.action === "silent") {
            if (global.settings?.autotyping || global.settings?.autovoice) {
                await sock.sendPresenceUpdate("paused", m.chat)
            }
            return
    ...
    // Trusted access di grup trust → bypass wajib register.
    if (!isPrivileged && !isRegistered(sender) && !bypassRegister) {
        if (global.settings?.autotyping || global.settings?.autovoice) {
            await sock.sendPresenceUpdate("paused", m.chat)
        }
        return Button.menu({
    ...
    // ─── Owner Only Check ───
    // Trusted access (user trust DI grup trust) boleh memakai command owner
    // KECUALI reboot & self — di luar itu tetap khusus owner.
    if (plugin.owner && !isCreator) {
        const trustedAllowed = isTrustedAccess && !isTrustBlockedCommand(command, plugin)
        if (!trustedAllowed) {
            if (isTrustedAccess) {
                return m.reply(
                    `╭━━━〔 🔒 AKSES DITOLAK 〕━━━⬣\n` +
                        `┃\n` +
                        `┃ Command *${command}* tidak\n` +
                        `┃ tersedia untuk user trust.\n` +
                        `┃ (Khusus owner: reboot & self.)\n` +
                        `┃\n` +
                        `╰━━━━━━━━━━━━━━━━━━⬣`
                )
            }
    ...
    if (!isPrivileged && !isFree && !premium) {
