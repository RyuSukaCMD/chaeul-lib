...
import { isGroupRegistered, isDisabled, isAdminOnly, isAllDisabled } from "./groupmanage.js"
...
export default async function Handler(sock, m) {
    ...
    if (m.isGroup && !isCreator) {
        ...
        if (!isGroupRegistered(m.chat)) {
            ...
            if (isAllDisabled(m.chat) && !recoverCmds.includes(command)) {
                return // semua command dimatikan → diamkan (kecuali group mgmt)
            }
    ...
    const registerWhitelist = [
        ...
        // Group management (admin, tak perlu registrasi user)
        "registergroup",
        "reggroup",
        "unregistergroup",
        "unregroup",
        ...
        "listdisabledcommand",
        "listdisable",
        // Group moderation (admin, tak perlu registrasi user)
        "del",
        "delete",
        "d",
        "close",
    ...
    const freeCommands = [
        ...
        "tokens",
        "saldo",
        // Group management & moderation (gratis, alat admin)
        "registergroup",
        "reggroup",
        "unregistergroup",
        "unregroup",
