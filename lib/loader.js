...
class Loader {
    ...
    /**
     * Nama utama (canonical) sebuah plugin = alias string pertama.
     * Dipakai agar "disable command" ikut mematikan SELURUH alias &
     * sub-command (tombol/regex) milik plugin yang sama.
     */
    primaryName(plugin) {
        if (!plugin || !Array.isArray(plugin.command)) return null
        const name = plugin.command.find((c) => typeof c === "string")
        return name ? name.toLowerCase() : null
    }

    /** Nama utama plugin pemilik sebuah command/alias/tombol. */
    resolvePrimary(command) {
        const plugin = this.get(command) || this.getButton(command)
        return this.primaryName(plugin)
    }

    /** Semua alias string milik plugin pemilik command tsb. */
    aliasesOf(command) {
        const plugin = this.get(command) || this.getButton(command)
        if (!plugin || !Array.isArray(plugin.command)) return []
        return plugin.command.filter((c) => typeof c === "string").map((c) => c.toLowerCase())
    }
