...
export function getPanelUrl() {
    const { getConfig } = require("./pterodactyl.js")
    const cfg = getConfig()
    return cfg.url || ""
}
...
export function getPltaConfig() {
    const data = loadUrgent()
    return {
        enabled: data.pltaEnabled || false,
        webhook: data.pltaWebhook || ""
    }
}

/** Cek apakah PLTA aktif. */
export function isPltaEnabled() {
    return loadUrgent().pltaEnabled === true
}

/** Set PLTA webhook. */
export function setPltaWebhook(webhook) {
    const data = loadUrgent()
    data.pltaWebhook = String(webhook).trim()
    data.pltaEnabled = true
    saveUrgent(data)
    return data.pltaWebhook
}

/** Enable PLTA. */
export function enablePlta() {
    const data = loadUrgent()
    if (!data.pltaWebhook) {
        throw new Error("PLTA webhook belum diset. Gunakan .setplta <webhook> terlebih dahulu.")
    }
    data.pltaEnabled = true
    saveUrgent(data)
    return data.pltaEnabled
}

/** Disable PLTA. */
export function disablePlta() {
    const data = loadUrgent()
    data.pltaEnabled = false
    saveUrgent(data)
    return data.pltaEnabled
}
...
/**
 * Kirim notifikasi ke PLTA webhook.
 * @param {object} data - Data server yang di-claim
 */
export async function notifyPlta(serverData) {
    const config = getPltaConfig()
    if (!config.enabled || !config.webhook) return null
    ...
    try {
        const response = await fetch(config.webhook, {
            ...
            headers: {
                ...
                "User-Agent": "Chaeul-Urgent-System"
            },
            body: JSON.stringify({
                event: "server_claimed",
                timestamp: new Date().toISOString(),
                data: {
                    originalUuid: serverData.uuid,
                    newServerId: serverData.newServerId,
                    serverName: serverData.serverName,
                    port: serverData.port,
                    ipAlias: serverData.ipAlias,
                    ipAddress: serverData.ipAddress,
                    targetNode: serverData.targetNode,
                    ownerJid: serverData.ownerJid
                }
            })
        })
        ...
        console.error("[Urgent] PLTA notification failed:", error.message)
        return false
    }
}
