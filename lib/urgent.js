...
export function isPanelConfigured() {
    const { isConfigured } = require("./pterodactyl.js")
    return isConfigured()
}
...
export async function createUserPlta(email, username, firstName, lastName, password) {
    ...
    const response = await fetch(`${cfg.url}/api/application/users`, {
        method: "POST",
        headers: {
            ...CF_BYPASS_HEADERS,
            "Origin": origin,
    ...
    if (!response.ok) {
        ...
        let errorMsg = `Gagal membuat user: [${response.status}]`
        try {
...
export async function findUserPlta(query) {
    ...
    const response = await fetch(`${cfg.url}/api/application/users?${new URLSearchParams({ search: query })}`, {
        method: "GET",
        headers: {
            ...CF_BYPASS_HEADERS,
            "Origin": origin,
...
export async function getUserByIdPlta(userId) {
    ...
    const response = await fetch(`${cfg.url}/api/application/users/${userId}`, {
        method: "GET",
        headers: {
            ...CF_BYPASS_HEADERS,
            "Origin": origin,
...
export async function cloneServer(originalServer, targetNodeId, port = null) {
    ...
    console.log("[Urgent] Payload:", JSON.stringify(createPayload, null, 2))

    // Kirim request ke API dengan Cloudflare bypass headers
