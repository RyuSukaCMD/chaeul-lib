...
/**
 * Make request to PLTA API with Cloudflare bypass.
 */
async function pltaRequest(endpoint, options = {}) {
    const cfg = getPltaConfig()
    ...
    const requestUrl = `${cfg.url}/api/application${endpoint}`
    ...
    try {
        ...
        if (error.response) {
            ...
            const body = error.response.body || errorText
            
            if (body.includes("cf-challenge") || body.includes("Cloudflare") || body.includes("Just a moment")) {
                throw new Error(`Cloudflare Protection: Gagal bypass PLTA.`)
            }
            
            throw new Error(`PLTA API Error [${status}]: ${body}`)
        }
        ...
        throw new Error(`PLTA API Error: ${errorText}`)
    }
}
...
export async function createUserPlta(email, username, firstName, lastName, password) {
    const result = await pltaRequest("/users", {
        method: "POST",
        body: {
            email: email,
            username: username,
...
export async function findUserPlta(query) {
    const result = await pltaRequest(`/users?${new URLSearchParams({ search: query })}`)
...
export async function getUserByIdPlta(userId) {
    try {
        const result = await pltaRequest(`/users/${userId}`)
        return result
    } catch {
        return null
    }
