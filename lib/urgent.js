...
//   "unknown"   → daftar allocation tidak bisa dibaca (panel error/lambat) —
//                 jangan blokir user; cloneServer tetap cek ulang secara otoritatif
export async function getPortState(nodeId, port) {
    let allocations
    try {
        allocations = unwrapArr(await getNodeAllocations(nodeId))
    } catch (error) {
        console.warn(`[Urgent] getPortState: gagal baca allocations node ${nodeId}: ${error.message}`)
        return { state: "unknown", allocationId: null, error: error.message }
    }
...
// ─── Clone UUID Tracking (ANTI INFINITE CLONE) ───
// Setiap server hasil clone memiliki UUID BARU. UUID tersebut dicatat di
// sini supaya server hasil clone TIDAK BISA di-clone lagi tanpa batas.

export function getClonedUUIDs() {
    return loadUrgent().clonedUUIDs || {}
}

export function registerCloneUUID(uuid, info = {}) {
    const key = String(uuid || "").toLowerCase()
    if (!key) return null
    const data = loadUrgent()
    if (!data.clonedUUIDs) data.clonedUUIDs = {}
    data.clonedUUIDs[key] = {
        uuid: key,
        originalUUID: info.originalUUID || null,
    ...
    saveUrgent(data)
    return data.clonedUUIDs[key]
}

// Cocok persis maupun prefix (uuid short), seperti pencarian server biasa.
export function isCloneUUID(uuid) {
    const input = String(uuid || "").toLowerCase()
    if (!input) return false
    const map = getClonedUUIDs()
    if (map[input]) return true
    if (input.length >= 8) {
        for (const stored of Object.keys(map)) {
            if (stored.startsWith(input)) return true
        }
    }
    return false
}

export function getCloneInfo(uuid) {
    const input = String(uuid || "").toLowerCase()
    const map = getClonedUUIDs()
    if (map[input]) return map[input]
    if (input.length >= 8) {
        for (const [stored, info] of Object.entries(map)) {
            if (stored.startsWith(input)) return info
        }
    }
    return null
}

export function deleteCloneUUID(uuid) {
    const data = loadUrgent()
    const key = String(uuid).toLowerCase()
    if (!data.clonedUUIDs || !data.clonedUUIDs[key]) return false
    delete data.clonedUUIDs[key]
    saveUrgent(data)
    return true
}
...
async function findOrCreateAllocation(targetNodeId, port, { ipAlias, ipAddress } = {}) {
    ...
    // (3) belum ada → BUAT allocation baru DULU (sesuai permintaan user),
    //     baru dipakai. Proses tahan banting:
    //     • Urutan IP calon: IP paling umum di node → IP konfigurasi urgent
    //       → hasil resolve DNS FQDN node → 0.0.0.0 (wildcard).
    //     • Tiap IP dicoba DENGAN alias dulu; kalau panel menolak alias,
    //       dicoba ulang TANPA alias.
    //     • Kalau response sukses tapi id tidak kebaca (bentuk response
    //       panel aneh), daftar allocation di-fetch ULANG untuk mencari
    //       allocation port ini yang kosong.
    const ipCounts = {}
    for (const a of allocations) {
        if (a.ip) ipCounts[a.ip] = (ipCounts[a.ip] || 0) + 1
    }
    const candidateIps = Object.keys(ipCounts).sort((x, y) => ipCounts[y] - ipCounts[x])

    if (ipAddress && ipAddress !== "0.0.0.0" && ipAddress !== "::") {
        candidateIps.push(ipAddress)
    }
    ...
    try {
        ...
        if (fqdn) {
            ...
            if (address) candidateIps.push(address)
        }
    } catch (error) {
        console.warn(`[Urgent] Gagal resolve IP node ${targetNodeId}: ${error.message}`)
    }

    candidateIps.push("0.0.0.0")

    // unik + rapikan prioritas
    const ipQueue = [...new Set(candidateIps.filter(Boolean))]

    // Satu kali percobaan POST. Hasil:
    //  { ok: true, id }                      → sukses, id ketemu
    //  { ok: true, id: null }                → POST diterima tapi id tak ketemu
    //  { ok: false, error, aliasSensitive? } → ditolak panel
    const tryCreate = async (allocIp, useAlias) => {
        const created = await createNodeAllocation(targetNodeId, {
            ip: allocIp,
            ports: [port],
            alias: useAlias ? ipAlias || undefined : undefined
        })

        let match = created.find((a) => Number(a.port ?? a.attributes?.port) === Number(port))
        let newId = match ? (match.id ?? match.attributes?.id) : null

        // Response sukses tapi id tidak kebaca → fetch ulang daftar allocation
        // (beberapa panel membalas bentuk aneh / 204 padahal allocation dibuat)
        if (!newId) {
            console.warn(`[Urgent] Response buat allocation tidak berisi id → fetch ulang...`)
            const fresh = unwrapArr(await getNodeAllocations(targetNodeId)).map((a) => ({
                id: a.id ?? a.attributes?.id,
                port: Number(a.port ?? a.attributes?.port),
                assigned: !!(a.assigned ?? a.attributes?.assigned)
            }))
            match = fresh.find((a) => a.port === Number(port) && !a.assigned)
    ...
    for (const allocIp of ipQueue) {
        for (const useAlias of [true, false]) {
            const aliasTag = useAlias && ipAlias ? ` (alias: ${ipAlias})` : ""
            console.log(`[Urgent] Port ${port} belum ada → BUAT allocation ${allocIp}:${port}${aliasTag}`)

            try {
                const res = await tryCreate(allocIp, useAlias)
                if (res.id) {
                    console.log(`[Urgent] Allocation baru dibuat: ID ${res.id} (${allocIp}:${port})`)
                    return { allocationId: res.id, created: true, ip: allocIp }
                }
                // POST diterima tapi allocation tak ketemu → JANGAN coba lagi di IP
                // yang sama (rawan duplikat) — lanjut kandidat IP berikutnya.
                attemptErrors.push(`${allocIp}${aliasTag}: sukses tapi allocation tidak terbaca`)
                console.warn(`[Urgent] ${allocIp}:${port} sukses tapi tidak terbaca → coba IP lain`)
                break
            } catch (error) {
                const reason = `${allocIp}${aliasTag}: ${error.message}`
                attemptErrors.push(reason)
                console.warn(`[Urgent] Buat allocation gagal (${reason})`)
                // 403 (read-only key) → percobaan lain tidak akan membantu
                if (/unauthorized|forbidden|403/i.test(error.message)) {
                    stopAll = true
                    break
                }
                // error lain (mis. 422 alias invalid) → coba tanpa alias / IP lain
            }
        }
        if (stopAll) break
    }

    const lastError = attemptErrors.at(-1) || "tidak diketahui"
    throw new Error(
        `Gagal menyiapkan allocation untuk port ${port} di node target: ${lastError}` +
            (/unauthorized|forbidden|403/i.test(lastError)
                ? "\n\n💡 Error 403 saat buat allocation = API key (PLTA) bersifat *Read-only*. Buat key Read & Write lalu set ulang dengan .setpterodactyl"
                : "\n\n_Panel menolak pembuatan allocation — coba port lain atau hubungi owner._")
    )
}
...
export async function cloneServer(originalServer, targetNodeId, port = null) {
    ...
    // ANTI INFINITE CLONE: server hasil clone tidak boleh di-clone ulang.
    if (isCloneUUID(serverUuid)) {
        const info = getCloneInfo(serverUuid)
        throw new Error(
            "Server ini adalah *hasil CLONE* dan tidak dapat di-clone lagi (anti infinite clone)." +
                (info?.originalUUID ? `\nUUID server asal: ${info.originalUUID}` : "")
        )
    }
