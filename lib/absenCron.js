...
function runCronOnce() {
    loadEnv()
    if (!enabled()) return
    // Sistem dinonaktifkan lewat .disable → cron tidak dijalankan.
    if (!isSystemEnabled("absencron")) return
...
export default function startAbsenCron(sock = null) {
    loadEnv()
    if (sock) startAbsentWarn(sock)
