...
export function isRegistered(sender) {
    const u = loadDB()[norm(sender)]
    // Terdaftar bila ada entri & tidak ditandai unregistered (backward-compatible:
    // entri lama tanpa flag dianggap terdaftar).
    return !!u && u.registered !== false
}
...
export function saveUser(sender, { name, gender, age }) {
    ...
    // Merge: pertahankan field lama (mis. data tambahan), set ulang registered.
    db[key] = {
        ...(db[key] || {}),
        name,
        gender,
        age,
        registered: true,
        registeredAt: db[key]?.registeredAt || Date.now()
    }
    delete db[key].unregisteredAt

    saveDB(db)

    // Push real-time ke website (lazy import agar tak circular).
    import("./license.js").then((m) => m.pushUser?.(key, name)).catch(() => {})
...
/**
 * Unregister TANPA menghapus data (RPG, nama, dll tetap utuh).
 * Hanya menandai user sebagai belum-terdaftar agar bisa daftar ulang.
 * Progres RPG (rpg.json) memang di DB terpisah → otomatis aman.
 */
export function unregisterUser(sender) {
    const db = loadDB()
    const key = norm(sender)
    if (db[key]) {
        db[key].registered = false
        db[key].unregisteredAt = Date.now()
        saveDB(db)
    }
}
