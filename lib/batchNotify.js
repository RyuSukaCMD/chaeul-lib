 ...
 * @param {string} [o.system]  key sistem (.disable) yang mengatur notif ini
 */
export function enqueue({
    ...
    // Notifikasi jenis ini dimatikan untuk grup tsb lewat .disable → buang.
    try {
        const key = system || (type === "welcome" || type === "goodbye" ? "welcome" : type)
        if (!canNotify(chat, key)) return
    } catch {}
