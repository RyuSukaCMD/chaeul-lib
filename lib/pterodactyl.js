...
async function pteroFetch(endpoint, options = {}) {
    ...
    const requestUrl = `${url}/application${endpoint}`
...
// ─── Buat allocation baru di node (POST /nodes/{id}/allocations) ───
// WAJIB dipakai sebelum membuat server bila port yang diinginkan belum
// punya allocation. Urutan yang benar: buat allocation dulu → pakai id-nya.
// Response allocation yang dibuat (sudah di-unwrap) — tahan terhadap dua
// bentuk response panel: {object:"list", data:[...]} maupun
// {object:"allocations", attributes:[...]}.
export async function createNodeAllocation(nodeId, { ip, ports, alias } = {}) {
    if (!ip) throw new Error("IP allocation tidak boleh kosong.")
    const portList = (Array.isArray(ports) ? ports : [ports]).map((p) => String(p))

    const body = { ip: String(ip), ports: portList }
    if (alias) body.alias = String(alias)

    const result = await pteroFetch(`/nodes/${nodeId}/allocations`, {
        method: "POST",
        body
    })

    // Bentuk 1: fractal list { object: "list", data: [...] } → unwrapList biasa
    const asList = unwrapList(result)
    if (asList.length) return asList

    // Bentuk 2: { object: "allocations", attributes: [...] (array polos) }
    if (result && Array.isArray(result.attributes)) {
        return result.attributes.map((a) => unwrapItem(a))
    }

    return []
}

// ─── Detail egg (+ variabel) — sumber docker_image, startup & environment ───
// GET /nests/{nestId}/eggs/{eggId}?include=variables
// Berguna untuk clone server persis sama: docker image & variabel egg yang
// belum ada di environment server akan diisi dari default egg.
export async function getEgg(nestId, eggId, includeVariables = true) {
    const query = includeVariables ? "?include=variables" : ""
    return unwrapItem(await pteroFetch(`/nests/${nestId}/eggs/${eggId}${query}`))
}

export async function getEggVariables(nestId, eggId) {
    const egg = await getEgg(nestId, eggId, true)
    // Setelah unwrap: relationships.variables = array variabel (flat)
...
// ─── Ping daemon/wings (TCP connect ke fqdn:daemon_listen) ───
export function pingDaemon(fqdn, port = 8080, timeout = 8000) {
    return new Promise((resolve) => {
        if (!fqdn) return resolve(false)
