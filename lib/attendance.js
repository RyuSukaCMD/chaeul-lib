...
/**
 * Port dari ensure_attendance($email, $username, $ptero_server_id):
 * buat record pertama bila belum ada, kalau sudah ada kembalikan expires_at.
 */
export function ensureAttendance(key, meta = {}) {
    const data = getAllAttendance()
    const k = String(key).toLowerCase()
    if (!data[k]) {
        const t = now()
