...
/** Broadcast tangkapan ikan → feed live di web. */
export function pushFishing({ name, fish, rarity, value, island } = {}) {
    push("/api/sync/fishing", { name, fish, rarity, value, island })
}
...
// Sinkron berkala (live-ish) — tiap 1 menit push snapshot penuh.
let syncTimer = null
...
export async function processNotifications(sock) {
    ...
    for (const n of notifs) {
        try {
            await sock.sendMessage(ownerJid, { text: n.text })
        } catch {}
