...
/**
 * Tanya website: apakah grup ini SUDAH terdaftar (punya lisensi aktif)?
 * Return { exists, license } atau { exists:false } bila web tak terjangkau.
 */
async function groupAlreadyRegistered(groupJid) {
    const c = cfg()
    if (!base() || !c.syncToken || !groupJid) return { exists: false }
    try {
        const { data } = await axios.post(
            `${base()}/api/provision/groupstatus`,
            { groupJid },
            { headers: { "x-sync-token": c.syncToken }, timeout: 15000 }
        )
        return { exists: !!data?.exists, license: data?.license || null }
    } catch {
        return { exists: false }
    }
}

/** Cek apakah BOT sudah menjadi anggota sebuah grup (via metadata). */
async function botInGroup(sock, groupJid) {
    if (!groupJid) return false
    try {
        const meta = await sock.groupMetadata(groupJid)
        const botId = (sock.user?.id || "").split(":")[0].replace(/[^0-9]/g, "")
        return !!meta?.participants?.some((p) =>
            String(p.id || "").replace(/[^0-9]/g, "").startsWith(botId)
        )
    } catch {
        return false
    }
...
async function joinGroupSafely(sock, code, knownJid = null) {
    ...
    for (let attempt = 1; attempt <= RETRY; attempt++) {
        try {
            ...
            // groupAcceptInvite falsy → bisa berarti:
            //  (a) sebenarnya sudah masuk (verifikasi metadata), ATAU
            //  (b) grup butuh PERSETUJUAN ADMIN → permintaan join terkirim (pending).
            if (knownJid && (await botInGroup(sock, knownJid))) {
                return { ok: true, groupJid: knownJid }
            }
            // Coba jalur request-join eksplisit (grup dengan approval).
            const reqRes = await tryRequestJoin(sock, code, knownJid)
            ...
            if (msg.includes("already") || msg.includes("409") || msg.includes("conflict")) {
                ...
                try {
                    ...
                    if (jid && (await botInGroup(sock, jid))) return { ok: true, groupJid: jid }
                    if (jid) return { ok: true, groupJid: jid }
                } catch {}
            }

            // Grup butuh PERSETUJUAN ADMIN untuk bergabung.
            // Baileys biasanya mengembalikan error 'not-authorized'/'forbidden' pada
            // grup ber-approval saat groupAcceptInvite. Kirim request join + tandai pending.
            if (
...
/**
 * Kirim PERMINTAAN bergabung untuk grup yang butuh persetujuan admin.
 * Return { ok:false, needsApproval:true, groupJid } bila permintaan terkirim,
 * atau null bila jalur ini tidak berlaku (biar caller lanjut logic lain).
 */
async function tryRequestJoin(sock, code, knownJid = null) {
    // Baileys terbaru punya sock.groupRequestParticipantsUpdate / join via request.
    // Tidak semua versi punya API khusus; bila groupAcceptInvite gagal karena
    // approval, permintaan join umumnya SUDAH terkirim ke admin. Jadi kita cukup
    // pastikan jid grup diketahui, lalu laporkan sebagai butuh persetujuan.
    let jid = knownJid
    try {
        if (!jid) {
            const info = await sock.groupGetInviteInfo(code)
    ...
    // Sudah terlanjur masuk? berarti bukan approval.
    if (jid && (await botInGroup(sock, jid))) return { ok: true, groupJid: jid }
    if (jid) {
        return {
            ok: false,
            needsApproval: true,
            groupJid: jid,
            reason: "Grup butuh persetujuan admin. Permintaan bergabung sudah dikirim — minta admin grup menyetujui bot."
        }
    }
    return null
}
...
/**
 * Selesaikan registrasi grup yang bot-nya SUDAH di dalam:
 * validasi max member, register lokal, laporkan sukses (lisensi terbit),
 * lalu sapa grup dengan masa aktif. Return report (truthy) bila selesai.
 */
async function finishRegister(sock, job, groupJid, groupName, members) {
    // Ambil metadata final
    try {
        const meta = await sock.groupMetadata(groupJid)
    ...
    // Auto-out bila melebihi batas (private)
    if (job.maxMembers && members != null && members > job.maxMembers) {
        ...
        await reportProvision(job.id, {
            success: false,
            status: "over_limit",
            reason: `Grup punya ${members} anggota (maks ${job.maxMembers}). Bot keluar dari grup.`,
            groupJid,
            groupName,
            members
        })
    ...
    await greetGroup(sock, groupJid, groupName, rep?.license?.expiresAt)
    return rep || true
}

/** Kirim sapaan "bot aktif" ke grup + masa aktif (bila ada). */
async function greetGroup(sock, groupJid, groupName, expiresAt) {
    let masaAktif = ""
    if (expiresAt) {
        const days = Math.max(0, Math.ceil((expiresAt - Date.now()) / 86400000))
        const tgl = new Date(expiresAt).toLocaleDateString("id-ID", {
            day: "numeric",
            month: "long",
            year: "numeric"
        })
...
export async function processProvisionJobs(sock) {
    ...
    for (const job of jobs) {
        ...
        try {
            ...
            // ── FITUR: DETEKSI GRUP SUDAH ADA ──
            // Bila grup ini sudah punya lisensi aktif (sudah dipasang bot), jangan
            // dobel-join. Laporkan agar admin tahu (order butuh perhatian).
            if (groupJid) {
                const gs = await groupAlreadyRegistered(groupJid)
                if (gs.exists) {
                    await reportProvision(job.id, {
                        success: false,
                        status: "already_exists",
                        reason: `Grup "${gs.license?.groupName || groupName || groupJid}" sudah terdaftar & memiliki lisensi aktif.`,
                        groupJid,
                        groupName: gs.license?.groupName || groupName,
                        members
                    })
                    try {
                        const ownerJid =
                        ...
                        await sock.sendMessage(ownerJid, {
                            text:
                                `ℹ️ *Grup sudah terdaftar* (order ${job.id})\n\n` +
                                `Grup: ${gs.license?.groupName || groupName || "-"}\n` +
                                `Grup ini sudah punya lisensi aktif — order tidak diproses ulang.`
                        })
                    } catch {}
                    continue
                }
                // Bot ternyata sudah di dalam grup (mis. di-invite manual sebelumnya).
                if (await botInGroup(sock, groupJid)) {
                    // Lewati join, langsung anggap sukses → lisensi terbit.
                    const rep = await finishRegister(sock, job, groupJid, groupName, members)
                    if (rep) continue
                }
            }

            // ── FITUR: AUTO-TOLAK bila anggota melebihi batas (private) ──
            if (job.maxMembers && members != null && members > job.maxMembers) {
                await reportProvision(job.id, {
                    success: false,
                    status: "over_limit",
                    reason: `Grup punya ${members} anggota (maks ${job.maxMembers} untuk paket private).`,
                    groupJid,
                    groupName,
                    members
                })
            ...
            // Join grup — dengan retry & fallback (restricted / approval)
            const joinRes = await joinGroupSafely(sock, code, groupJid)

            // ── FITUR: DETEKSI GRUP BUTUH PERSETUJUAN ADMIN ──
            if (joinRes.needsApproval) {
                ...
                try {
                    ...
                    await sock.sendMessage(ownerJid, {
                        text:
                            `⏳ *Grup butuh persetujuan admin* (order ${job.id})\n\n` +
                            `Grup: ${groupName || "-"}\n` +
                            `Bot sudah mengirim permintaan bergabung. Minta admin grup menyetujui bot, ` +
                            `nanti bot otomatis aktif begitu diterima.`
                    })
                } catch {}
                continue
            }
            ...
            if (!joinRes.ok) {
                await reportProvision(job.id, {
                    success: false,
                    status: joinRes.restricted ? "restricted" : "failed",
                    reason: joinRes.reason,
                    groupJid,
                    groupName,
                    members
...
// ═══════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════
//  AUTO-OUT: PRIVATE GROUP MELEBIHI BATAS ANGGOTA
//  Untuk paket "private" (maks N anggota), bila setelah bot masuk jumlah
//  anggota BERTAMBAH melewati batas → bot memperingatkan lalu keluar
//  (dengan jeda toleransi singkat agar admin sempat merapikan).
//  Cache limit per grup agar tak query web tiap event.
// ═══════════════════════════════════════════════════════════
const groupLimitCache = new Map() // jid -> { maxMembers, at }
const LIMIT_TTL = 5 * 60 * 1000
const overLimitWarned = new Set() // jid yang sudah diperingatkan

async function getGroupLimit(groupJid) {
    const hit = groupLimitCache.get(groupJid)
    if (hit && Date.now() - hit.at < LIMIT_TTL) return hit.maxMembers
    const gs = await groupAlreadyRegistered(groupJid)
    const max = gs?.exists ? (gs.license?.maxMembers ?? null) : null
    groupLimitCache.set(groupJid, { maxMembers: max, at: Date.now() })
    return max
}

export function startMemberLimitWatcher(sock) {
    sock.ev.on("group-participants.update", async (data) => {
        try {
            ...
            const max = await getGroupLimit(groupJid)
            if (!max || max <= 0) return // tanpa batas (public / tak terdaftar)

            const meta = await sock.groupMetadata(groupJid)
            const members = meta?.participants?.length ?? 0
            if (members <= max) {
                overLimitWarned.delete(groupJid)
                return
            }

            // Pertama kali lewat batas → peringatkan, beri 60 detik toleransi.
            if (!overLimitWarned.has(groupJid)) {
                overLimitWarned.add(groupJid)
                try {
                    await sock.sendMessage(groupJid, {
                        text:
                            `⚠️ *Batas Anggota Terlampaui*\n\n` +
                            `Paket *Private* grup ini maksimal *${max} anggota*, ` +
                            `sekarang ada *${members}*.\n\n` +
                            `Kurangi anggota dalam *60 detik* atau bot akan keluar otomatis. ` +
                            `Untuk anggota lebih banyak, upgrade ke paket *Public*.`
                    })
                } catch {}
                setTimeout(async () => {
                    try {
                        const m2 = await sock.groupMetadata(groupJid)
                        const now = m2?.participants?.length ?? 0
                        const lim = await getGroupLimit(groupJid)
                        if (lim && now > lim) {
                            await sock.sendMessage(groupJid, {
                                text: `⛔ Anggota masih melebihi batas (${now}/${lim}). Bot keluar dari grup. Terima kasih.`
                            }).catch(() => {})
...
export function startGroupJoinWatcher(sock) {
    sock.ev.on("group-participants.update", async (data) => {
        try {
            ...
            // Metadata grup (butuh untuk cek batas & sapaan)
            let groupName = ""
            let members = null
            try {
                const meta = await sock.groupMetadata(groupJid)
            ...
            // ── FITUR: DETEKSI GRUP SUDAH ADA (terdaftar di web) ──
            // Bila grup sudah punya lisensi aktif, cukup pastikan command aktif,
            // JANGAN klaim job / kirim sapaan ulang.
            const gs = await groupAlreadyRegistered(groupJid)
            if (gs.exists) {
                try {
                    const gm = await import("./groupmanage.js")
                ...
                // Tetap tegakkan batas anggota bila private.
                if (gs.license?.maxMembers && members != null && members > gs.license.maxMembers) {
                    try {
                        await sock.sendMessage(groupJid, {
                            text: `⛔ Grup melebihi batas paket private (${members}/${gs.license.maxMembers}). Bot keluar.`
                        })
            ...
            // Sudah terdaftar lokal? jangan sapa ulang.
            let already = false
            try {
                const gm = await import("./groupmanage.js")
