...
//  100% API publik (tanpa StarNova). Provider (urut prioritas):
//   1. pollinations.ai  (POST, dukung system prompt + riwayat) — utama
//   2. davidcyriltech   (gpt3)                                  — fallback
//   3. vreden/fastrest  (chat)                                  — fallback
//  Bila satu gagal, otomatis coba berikutnya.
//  Gambar AI lewat pollinations (gratis, tanpa key).
// ═══════════════════════════════════════════════════════════
...
// ─── Provider: vreden AI (stateless) ───
async function viaVreden(prompt) {
    const { data } = await axios.get(
        `https://api.vreden.my.id/api/artificial/gpt?prompt=${encodeURIComponent(prompt)}`,
        { timeout: 60000, headers: { "User-Agent": UA } }
    )
    const out = data?.result?.message || data?.result || data?.message
...
export async function askAI(prompt, opt = {}) {
    ...
    for (const attempt of [
        () => viaPollinations(messages), // utama (gratis, dukung riwayat)
        () => viaDavid(flat),
        () => viaVreden(flat)
    ]) {
        try {
...
/**
 * Ambil URL gambar AI (pollinations, gratis tanpa key). Return string URL.
 */
