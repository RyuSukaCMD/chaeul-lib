...
export async function ytmp3(url) {
    const data = await getJSON(`${DAVID}/youtube/mp3?url=${encodeURIComponent(url)}`, {
        timeout: 90000
    })
    const audio = data?.download_url || data?.result?.download_url || data?.url
    if (!data?.success && !audio) throw new Error("Gagal mengambil YouTube MP3.")
    return {
...
export async function ytmp4(url) {
    const data = await getJSON(`${DAVID}/youtube/mp4?url=${encodeURIComponent(url)}`, {
        timeout: 90000
    })
    const r = data?.result || data
    const video = r?.url || r?.download_url || r?.video
